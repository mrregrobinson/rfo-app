...
- **Risk Management** (`/risk`) — the Enterprise Risk Register: risk categories grouped
  ...
  profile dashboard, a Family-Council PDF report, an optional web-search base-rate
...
- **Maturity Assessment** (`/maturity`) — periodic assessment of family-office
  ...
  same worksheet). Every service in every round also gets a web-search
  **benchmark** (1–5, "what would move us up") and the round gets a written
  **synthesis**. Rounds run `draft → open → closed`; at the start of a cycle refreshed
  wording can be proposed for the editable level descriptors, reviewed by an admin
...
- **Web research + PDF auto-extraction features**: these used to call
  ...
  them in manually instead of them being auto-filled.
...
### Enabling live web research / PDF extraction
...
- `risk_probability_lookups` — cached web-search base-rate estimates
...
- `maturity_service_groups` / `maturity_services` / `maturity_level_labels` /
  ...
  documents: annual meeting decks and notes, 2024-2026) fed into every benchmark call for
  ...
  artifacts/initiatives, never a judgment of maturity level — that's derived separately
  (and by the family). Before migration 043, `description` was always empty on every
  ...
  /api/maturity/services/:id`, surfaced in the service drawer), same "no review
...
- `POST /api/maturity/rounds/:id/suggest-wording` (admin-only, any round that isn't
  closed — "Refresh wording" on the Manage tab): one web-search call
  ...
  to open each service's drawer to find them; the per-service drawer still shows its own
  subset too, for reviewing in context. `maturity_descriptor_suggestions` (migration 033)
  holds the level-wording queue; `maturity_question_suggestions` (migration 039) holds the
  question-wording queue, keyed to a specific `maturity_questions` row via `question_id`
  (not by sort position, since that could drift). Before migration 039, all 16 services
  reused the exact same templated question text with only the service name substituted
  in — this is what lets a service's questions actually reflect that service, not a
  generic shape.
- `maturity_questions` / `maturity_responses` / `maturity_service_scores` — the concise,
  ...
  at any time, regardless of round status and with no dependency on anything having reviewed
...
- `maturity_benchmarks` — per (round, service) web-search read
  ...
  — an independent read of where THIS family likely sits, reasoned from the
  ...
  together — the family's self-score, assessed_level, and peer_level — rather
...
- Maturity report PDFs are built by `server/maturity-report.js` (`pdfkit` +
  ...
  by-service bar chart can be exported as pure family self-assessment or with the
  independent read and the peer benchmark alongside it. The chart gets its own page in the PDF, sized
  ...
  each service's write-up, every distinct statement (family mean, independent read, peer
  ...
  harder to read, not easier; the underlying grounding is still what feeds every benchmark
