...
// an independent read of where the family sits (assessedLevel) and its
...
async function barPng(model, showBenchmark) {
  ...
  if (showBenchmark) {
    datasets.push(
      { label: "Independent read", data: services.map((s) => (s.benchmark ? s.benchmark.assessedLevel : null)), backgroundColor: 'rgba(42,125,123,0.85)', barPercentage: 0.7, categoryPercentage: 0.8 },
      { label: 'Peer benchmark', data: services.map((s) => (s.benchmark ? s.benchmark.peerLevel : null)), backgroundColor: 'rgba(201,168,76,0.85)', barPercentage: 0.7, categoryPercentage: 0.8 }
    );
  }
  const width = 980;
    ...
    options: {
      ...
      plugins: {
        ...
        title: {
          ...
          text: showBenchmark
            ? "Maturity by service — family mean vs. independent read vs. peer benchmark (1–5)"
...
async function buildMaturityReportPdf(model, opts = {}) {
  ...
  const coverStats = [
    ...
    ...(showBenchmark ? [
      { label: "Independent read", value: fmt(assessedMean) },
  ...
  for (const g of groups) {
    ...
    for (const s of rows) {
      ...
      if (showBenchmark && assessedLvl != null) {
        doc.moveDown(0.3);
        doc.fillColor(INK).text(`Independent read: ${fmt(assessedLvl)} (${labelName(assessedLvl)})`);
      ...
      if (showBenchmark && bench != null) {
        doc.moveDown(0.3);
        doc.fillColor(INK).text(`Peer benchmark ${fmt(bench)} (${labelName(bench)})` + (gap != null ? `   ·   gap vs family ${gap > 0 ? '+' : ''}${gap.toFixed(1)}` : ''));
