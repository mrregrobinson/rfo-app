...
// 16 services x 1-5) AND "doing the right things" (Capital Consciousness — a standalone,
// once-per-round 1-7 read on the level of awareness the family is deciding from right
// now, independent of any one service), the highest-value priorities, and a short peer
// comparison.
async function synthesizeMaturityRound({ services, consciousness, familyContext }) {
  const today = new Date().toLocaleDateString('en-CA', { year: 'numeric', month: 'long', day: 'numeric' });
  const systemPrompt = `You are advising a Canadian single-family office after a periodic maturity assessment. Two separate reads are taken each round: (1) operational maturity — each of 16 services scored 1-5 on how well it is run, i.e. whether the family office is "doing things right"; and (2) Capital Consciousness — a single family-wide 1-7 read, taken once per round (not per service), on what level of awareness the family is currently deciding from (1 survival/fear, 2 accumulation/competition, 3 protection/care, 4 system/integration, 5 mirror/values, 6 force/impact, 7 freedom/non-attachment; the circle of responsibility widens and attachment loosens up the scale, crossing around Level 4) — i.e. whether they are "doing the right things". Each family member picks the ONE of 7 short plain-language statements that best reflects the family today; the level is that statement's level, not a self-label on the named framework. Use web search only for peer-comparison colour, not for the family's own numbers. Today is ${today}.`;
  const userPrompt = `Family context: ${familyContext}

Per-service operational maturity: ${JSON.stringify(services)}
Capital Consciousness (family-wide, once per round): ${JSON.stringify(consciousness)}
